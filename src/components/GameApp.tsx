    ...
    <div className="pointer-events-none absolute inset-0 select-none text-white">
      ...
      <div className="absolute left-1/2 top-4 flex -translate-x-1/2 gap-2">
        ...
        <Meter
          label="Engine"
          value={heat * 100}
          color={heat > 0.82 ? "#fb7185" : "#fbbf24"}
          text={String(Math.round(heat * 100)) + "%"}
        />
        <Meter
          label="Voyage"
          value={progress * 100}
          color="#67e8f9"
          text={String(Math.round(progress * 100)) + "%"}
        />
      </div>
      ...
      <div className="absolute bottom-4 left-4 max-w-xs space-y-2">
            ...
            {myRoles.length ? (
              myRoles.map((roleKey) => {
                ...
                return (
                  <span
                    key={roleKey}
                    className="rounded-full bg-cyan-300/15 px-2 py-1 text-xs font-bold text-cyan-200"
                  >
                    {role?.name ?? roleKey}
                  </span>
                );
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-950/75 px-4 py-3 text-xs text-white/65 shadow-xl backdrop-blur-md">
          <div className="mb-1 font-bold text-white/90">
            Crew: {roster.length ? roster.map((member) => member.name).join(" · ") : "connecting…"}
          </div>
          <div>
            {Math.round(speed)} knots · {leaks} leak{leaks === 1 ? "" : "s"} · {crates} crates ·{" "}
            {pax} passengers
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-4 right-4 rounded-2xl border border-white/10 bg-slate-950/75 px-4 py-3 text-right text-xs text-white/60 shadow-xl backdrop-blur-md">
        <div>
          <span className="font-bold text-white/90">WASD</span> move / steer
        </div>
        <div>
          <span className="font-bold text-white/90">E</span> interact ·{" "}
        ...
        <div className="pointer-events-auto absolute inset-0 flex items-center justify-center bg-slate-950/35 backdrop-blur-[2px]">
          <div className="mx-4 max-w-md rounded-3xl border border-white/15 bg-slate-950/90 p-8 text-center shadow-2xl">
            <div className="text-xs font-black uppercase tracking-[0.3em] text-cyan-300">
              {phase === "docked" ? "Safe arrival" : "The ferry sank"}
            </div>
            <h2 className="mt-3 text-4xl font-black tracking-tight text-white">
              {phase === "docked" ? "You made it!" : "Crew overboard."}
            </h2>
            <p className="mt-3 text-sm leading-6 text-white/65">
              {phase === "docked"
                ? "Everyone pulled together and reached the dock."
                : "The hull integrity reached zero. Try a slower, less chaotic crossing."}
            </p>
            <button
              type="button"
              onClick={() => {
              ...
              className="mt-6 rounded-xl bg-cyan-300 px-5 py-3 text-sm font-black uppercase tracking-wider text-slate-950 transition hover:bg-cyan-200"
            >
              Back to harbor
            </button>
          </div>
        ...
        <div className="grid md:grid-cols-[1.15fr_0.85fr]">
          <section className="relative min-h-[560px] overflow-hidden p-8 sm:p-12">
            ...
            <div className="relative flex h-full flex-col justify-between">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.25em] text-cyan-200">
                  Co-op physics voyage
                </div>
                <h1 className="mt-7 max-w-lg text-6xl font-black leading-[0.88] tracking-[-0.07em] text-white sm:text-8xl">
                  Wobble
                  <br />
                  <span className="text-cyan-300">Ferry</span>
                </h1>
                <p className="mt-7 max-w-md text-base leading-7 text-white/60">
                  A tiny crew. One wildly unstable boat. Keep the engine cool, patch the leaks, and
                  do not let the passengers discover the edge.
                </p>
              </div>
              <div className="grid max-w-md grid-cols-2 gap-3 text-xs text-white/55">
                <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                  <div className="mb-1 text-lg">⚓</div>
                  <b className="text-white/90">4 roles</b>
                  <br />
                  Every station matters.
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                  <div className="mb-1 text-lg">🌊</div>
                  <b className="text-white/90">2–4 players</b>
                  <br />
                  Share one room link.
                </div>
              </div>
          ...
          <section className="flex flex-col justify-center border-t border-white/10 bg-white/[0.035] p-8 sm:p-12 md:border-l md:border-t-0">
            ...
            <p className="mt-3 text-sm leading-6 text-white/55">
              Use the same room code as your friends. You can copy the invite link after boarding.
            </p>
            <form
            ...
            >
              <label className="block text-xs font-bold uppercase tracking-[0.16em] text-white/55">
                ...
                <input
                  ...
                  placeholder="e.g. Captain Noodle"
                  maxLength={18}
                  className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/25 focus:border-cyan-300/60"
                  autoFocus
                />
              </label>
              <label className="block text-xs font-bold uppercase tracking-[0.16em] text-white/55">
                ...
                <input
                  ...
                  maxLength={8}
                  className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm uppercase tracking-[0.2em] text-white outline-none transition placeholder:normal-case placeholder:tracking-normal placeholder:text-white/25 focus:border-cyan-300/60"
                />
              </label>
              <button
                type="submit"
                className="w-full rounded-xl bg-cyan-300 px-5 py-3.5 text-sm font-black uppercase tracking-[0.14em] text-slate-950 transition hover:bg-cyan-200"
              >
                Board the ferry
              </button>
            </form>
            <div className="mt-6 flex items-center gap-2 text-[11px] text-white/35">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-300" /> Live rooms use Supabase
              Realtime.
            </div>
          </section>
        </div>
